...
export default function MeetingsIndex() {
  ...
  const eventTypes = ['all', 'meeting', 'workshop', 'guest_speaker', 'conference', 'movie_night', 'recruitment', 'event', 'holiday', 'study_event', 'finals'];
  ...
  const filterWeeks = (weeks) => {
    if (activeFilter === 'all') return weeks;
    return weeks.map(week => ({
      ...week,
      events: week.events.filter(ev => {
        const normalized = ev.kind?.toLowerCase().replace(/\s+/g, '_');
  ...
  return (
    <div className="main-content animate-fade-in" style={{ color: 'var(--text)' }}>
      ...
      <div className="meeting-nav">
        <button className="date-link btn btn-ghost" onClick={() => navigate('/')} aria-label="Go to Home">← Home</button>
      </div>
      ...
      <div className="filter-bar">
        {eventTypes.map(type => (
          <button
            key={type}
            className={`filter-btn ${type === activeFilter ? 'active' : ''} ${type === 'all' ? 'filter-btn-all' : ''}`}
                ...
                <ul style={{ marginTop: 6 }}>
                  {m.events.map((ev, i) => (
                    <li key={i} style={{ margin: '6px 0' }}>
                      {ev.kind && (
                        <span className={getBadgeClass(ev.kind)} style={{ marginRight: '8px' }}>
                          {ev.kind}
                        </span>
                      )}
                      {ev.kind === 'Recruitment' ? (
