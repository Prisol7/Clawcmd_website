...
export default function MeetingDetail() {
  ...
  useEffect(() => {
    ...
    const items = document.querySelectorAll('.meetingli');
    items.forEach((item, index) => {
      item.classList.add('will-animate', 'animate-fade-in', `stagger-${Math.min(index + 1, 10)}`);
    });
  }, [meeting]);
  ...
  return (
    <div className="main-content animate-fade-in">
      ...
      <div className="meeting-card animate-fade-in-scale">
              ...
              <li className="meetingli" key={i}>
                {ev.type && (
                  <div style={{ marginBottom: '8px' }}>
                    <span className={getBadgeClass(ev.type)}>{ev.type}</span>
                  </div>
                )}
      ...
      <div className="meeting-nav">
        <button className="date-link btn btn-secondary" disabled={!prevId} onClick={() => go(prevId)}>← Prev</button>
        <button className="date-link btn btn-primary" onClick={() => navigate('/meetings')}>All Weeks</button>
        <button className="date-link btn btn-secondary" disabled={!nextId} onClick={() => go(nextId)}>Next →</button>
      </div>
      <button className="date-link btn btn-ghost" onClick={() => navigate('/')}>← Back</button>
    </div>
  );
}
